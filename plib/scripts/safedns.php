...
function delete_plesk_missing_record_from_safedns($api_url,$zone_name,$pleskrecords,$records_array) {
        $records_arrayx=json_encode($records_array);
        if (strcasecmp($records_arrayx, 'NULL') == 0) {
...
//            echo " Matching Safedns Record: "; echo var_dump($safedns_record);
            ...
            foreach ($pleskrecords as $variablerr) {
        //        echo "Plesk Record: ";echo $variablerr['type']; echo $variablerr['host'];
                // 0 - ID , 1 - NAME , 2 - TYPE , 3 - CONTENT, 4 - OPT
//                echo "Name Vars:\n";
//                echo $safedns_record[1]."\n";
//                echo rtrim($variablerr['host'], ".")."\n";
//                echo "Type Vars\n";
//                echo $safedns_record[2]."\n";
//                echo $variablerr['type']."\n";
                if (strcasecmp($safedns_record[1], rtrim($variablerr['host'], ".")) == 0) {
//                    echo "Delete function - Matched name";
                    if (strcasecmp($safedns_record[2], $variablerr['type']) == 0) {
...
//           Leave NS and SOA Records alone. Delete anything else
                if (strcasecmp($safedns_record[2], 'NS') == 0) {
                    //echo "Not deleting NS Record\n"; 
                } elseif (strcasecmp($safedns_record[2], 'SOA') == 0) {
                    //echo "Not deleting SOA Record\n";
                } else {
                    echo "Deleting Record from SafeDNS, It no longer exists in plesk:\n : id- ".$safedns_record[0]."zone- ".var_dump($zone_name)." name- ".$safedns_record[1]." type- ".$safedns_record[2]." content- ".$safedns_record[3]."\n";
...
//                echo "SAFEDNS API Doesn't support ".$variablerr['type']." Records.\n";
        ...
        delete_plesk_missing_record_from_safedns($api_url,$data['zone']['name'],$data['zone']['rr'],$records_array);
        //     - 
        break;
    case 'delete':
// A
